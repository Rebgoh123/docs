...
class Edit extends React.Component {
    ...
    componentWillMount(){
        ...
        this.state.breadcrumb = [ {name:"Home", link:"#"} , {name:"Form", link:"/web/form"},  {name:params.formId, link:"/web/form/" + params.formId}];
        this.props.dispatch(modalActions.get_all_active())
        this.props.dispatch(formActions.get_form_type())
        this.props.dispatch(formActions.get_id_form(params.formId));
...
console.log( nextProps.template)
    ...
    addItem = (id,type) => {
       ...
       // var lengthItem = newItems.length.toString();
        ...
        var lengthItem = parseInt(newItems[newItems.length - 1]['id']) + 1;
    ...
    handleSubmit(event) {
        ...
        if(isFormValid(this.validators)) {
            ...
            this.props.dispatch(formActions.update(new_form))

            // history.push({
            //     pathname: '/form/add-on-template',
    ...
    handleTemplate(event) {
        ...
        if(isFormValid(this.validators)) {
            ...
            const {form, newItems,templateExist} = this.state;
            var new_form = [form, newItems]


           if(templateExist.length >= 1){
               history.push({
                   pathname: '/web/template/get/' + templateExist[0]['template_id'],
                   state: { detail: new_form }
               })
           }else{
               history.push({
    ...
    render() {
        const { breadcrumb,form,newItems,templateExist} = this.state;
        const { newItem,modals,types  } = this.props;
            ...
            <div className="display">
                <div className="content">
                    ...
                    <div className="div-menu-header">
                        ...
                        <MaterialUiDialog additional="View Template" handleSubmitAdditional={(e) => this.handleTemplate(e)} type="update" title="Update Form" submit={(e) => this.handleSubmit(e)}
                                              content="
                                               • Click SAVE to update existing form <br>
                                               • Click TEMPLATE to view template attached / add template to the form <br>
                                               • Click CLOSE to carry on editing the form"/>


                        <MaterialUiButton onSubmit={(e) => this.handleTemplate(e)}  title={templateExist !==undefined && templateExist.length >= 1 ? "Edit Template" : "Add Template"} style={{marginLeft: '20px', color:'#fff', backgroundColor:'#000'}}/>
                    ...
                    <Grid container spacing={24}>
                        <Grid item xs={8}>
                            ...
                            {newItem !== null ?
                                <Target newItems={newItems}>

                                    {newItems.length === 0 ?
                                        <div className="emptyPreview">
                                            <Typography variant="title" className="menu-header"
                                                        style={{color: '#DB5461'}}>
                                                Instruction
                                            </Typography>
                                            1) Fill up the Title and Description of the form in the Current tab from the
                                            right. <br/>
                        ...
                        <Grid item xs>
                            ...
                            { modals !== undefined && types!==undefined && form!==undefined ?
...
export default DragDropContext(HTML5Backend)(connectedHomePage);